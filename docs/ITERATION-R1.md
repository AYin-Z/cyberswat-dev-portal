...
**目标**：成员从"能登录"到"有主页、有资料、有归属感"；系统从"demo 级凭证"到"可正式对外"。
...
```
后端:
- GET /api/members/:id 返回扩展: projects（ProjectMember join → Project: name/status/role/joinedAt）+ ideas 数量
- 脱敏: 项目名/状态公开，不泄露内部任务详情

前端:
- MembersView 卡片 hover/详情页: 显示 "参与 N 个项目" + 项目名列表
- 可选: 成员详情路由 /members/:id（卡片点击展开）
```
...
- [ ] 端到端验证：编辑资料 → 成员页生效 → 通知实时到达 → 旧 token 401
