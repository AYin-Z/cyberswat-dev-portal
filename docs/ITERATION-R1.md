...
```
数据/词表 (2026-08-15 拍板: 两级词表):
- 分类级: Web前端/后端/数据库/安全/算法/AI/运维/测试/产品/设计...
- 技术级: Vue/NestJS/PostgreSQL/Prisma/Docker...
- 匹配用技术级交集, 展示用分类级; 词表开放加词后治理 (申请+审批)
- 数据模型: core_skill_categories + core_skills(技术, 挂分类) + 用户技能多对多
...
- [ ] 全部交付上线 dev.cyberswat.cn
- [ ] 生产凭证为强随机值（env 文件 600，不入库）+ pg_dump 每日备份（保留 30 天）实测恢复
- [ ] e2e 测试基建跑通（~30 核心用例：认证/权限/匹配/审批/项目级校验）
- [ ] 端到端验证：成员填技能 → 发点子 → 匹配成员收到定向通知 → 点击跳点子详情
- [ ] 两级词表入库（分类→技术），匹配用技术级交集
- [ ] 匹配通知频率限制：每人每天 ≤3 条 + 可关闭
